  ...
  _.forEach(routes, routeInfo => {
    ...
    if (routeInfo.middlewareType == 'blueprint') {
      ...
      let parameters = [...(template.parameters || [])]
        ...
        .flatMap(p => {
          if (p === 'attributeFilter') {
            let attributes = routeInfo.modelInfo.attributes;
            return _.map(attributes, (attributeInfo, key) => {
              return {
                in: 'query',
                name: key,
                required: false,
                scehma: generateAttributeSchema(attributeInfo),
    ...
    if (routeInfo.patternVariables) {
      ...
      let resolved = _.map(pathEntry.parameters, p => {
        let ref = p['$ref'];
        if (!ref) return p;
